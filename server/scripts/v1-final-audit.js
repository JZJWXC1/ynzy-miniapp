...
const criticalScripts = [
  ...
  'server/scripts/match-chat-session-isolation-v1-test.js',
  'server/scripts/match-chat-request-voice-decoupling-test.js',
  'server/scripts/listing-detail-operation-session-v1-test.js',
  'server/scripts/my-listings-operation-session-v1-test.js',
  'server/scripts/pending-filter-session-v1-test.js',
  'server/scripts/upload-commission-sync-v1-test.js',
...
function checkPausedReportDealHistoryContract() {
  ...
  assertOk(detailWxml.includes('bindtap="callLandlord"'), '详情页必须为非公司房源保留联系房东按钮')
  assertOk(detailWxml.includes('wx:for="{{listing.companyContactPhones}}"'), '公司房源详情必须逐项展示全部服务端统一号码')
  const callButtonAt = detailWxml.indexOf('bindtap="callLandlord"')
  const callButtonStart = detailWxml.lastIndexOf('<button', callButtonAt)
  const callButtonEnd = detailWxml.indexOf('</button>', callButtonAt)
  const callButtonBlock = detailWxml.slice(callButtonStart, callButtonEnd + '</button>'.length)
  assertOk(callButtonStart >= 0 && callButtonEnd > callButtonAt, '必须能精确定位非公司联系房东按钮')
  assertOk(callButtonBlock.includes('!listing.companyListing'), '公司房源不得新增联系房东按钮')
