...
assert(detailJs.includes('const canShareVideo = Boolean(listing && listing.videoUrl)'), '游客只要拿到公开视频地址就必须可以转发，不能绑定登录身份')
assert(detailJs.includes('if (!currentAuthToken()) return Promise.resolve(null)'), '游客转发不得调用受保护的服务端留痕接口')
assert(detailJs.includes('recordVideoFileShareBestEffort'), '登录用户的视频留痕必须与实际发送解耦')
assert(detailJs.includes('saveListingVideo'), '详情页必须提供游客可用的独立保存视频入口')
assert(/bindtap="saveListingVideo"/.test(detailWxml), '详情页必须渲染独立保存视频按钮')
assert(!detailJs.includes('请先登录内部中介账号后再转发'), '游客公开视频转发不得再提示登录')
