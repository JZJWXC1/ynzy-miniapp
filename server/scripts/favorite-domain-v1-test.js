...
// 6) 路由身份必须来自 token；收藏 PUT/DELETE 不解析或信任客户端身份正文。
{
  ...
  assert.ok((routeBlock.match(/updateMiniDb\(req,/g) || []).length >= 2, '收藏 PUT/DELETE 必须统一走数据库写锁内 fresh 验签 helper')
  assert.ok(/function updateMiniDb\(req,\s*mutator\)[\s\S]*miniAuthContextFromRequest\(req, nextDb\)/.test(source), 'fresh 验签 helper 必须从锁内最新数据库解析身份')
