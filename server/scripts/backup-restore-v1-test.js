...
const EXPECTED_COUNTS = { listings: 3, users: 2, reports: 1, deals: 2, commissionRecords: 1, footprints: 4 }
...
function run() {
  ...
  // 3) 失败场景二：恢复数量校验不符 → RESTORE_MISMATCH。
  {
    ...
    const stageDir = path.join(dir, 'backups')
    const db = sampleDb()
    // 故意构造：meta 记 listings=999，但 db 实际只有 3 条。
    backup.writeEnvelopeFile({
      ...
      meta: {
        ...
        createdAtMs: NOW,
        counts: { listings: 999, users: 2, reports: 1, deals: 2, commissionRecords: 1, footprints: 4 },
  ...
  // 4) 失败场景三：最近备份超 24 小时 → BACKUP_STALE。
  {
  ...
  // 5) 保留策略：保留 30 天内、清理过期、无视非备份命名文件。
  {
  ...
  // 6) 篡改检测：翻转一个字节，GCM 认证失败 → 演练失败并告警。
  {
  ...
  // 8) 跨备份计数回归：源被清空后，空库备份必须被拦（BACKUP_EMPTY_SOURCE）而非静默判成功。
  {
  ...
  // 9) 文件名时间戳越界必须判非法（防未来时间戳抑制超期告警 / 逃过保留清理）。
  {
  ...
  // 10) 恢复演练不残留明文：未传 tempDir 时自建临时目录用完即清；传入时才保留产物。
  {
  ...
  // 11) 异地目标门禁：默认缺 BACKUP_REMOTE_CMD 必须失败；只有显式 allowLocalOnly 才允许仅本地成功。
  {
    const mk = (tag) => {
