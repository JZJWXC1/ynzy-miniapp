...
`test:v1` 稳定排序运行全部 `*-test.js`，明确排除 `smoke-test.js`，即使前面有失败也始终执行 `v1-final-audit.js`，最后按失败总数非零退出。Mock/预览专项同时锁定游客助手只返回公司房源、详情 404/结构化失效态、动态佣金、行政区筛选、游客今日任务免请求、媒体字段、当前账号分佣记录与地图小区聚合 DTO；页面专项锁定共享视频登录返回、地图登录空态、无悬空分隔点、导航栈满回退和足迹单次刷新。
