...
- 公共层不只按字段名删除，还对所有可展示/搜索字符串执行同一套值级投影：全角、中文/金融数字、零宽/组合字符、emoji、任意标点、字母或汉字拆分的电话与结构化精确地址都不能夹带到标题、板块、小区、户型、标签、地图、附近、收藏、匹配、LLM 或飞书公开快照。清洗会保留完整的合法数字业务 token（如板块、地铁、分钟、公交、户型、面积、年份和租金），但不能用长数字尾部伪装成短地铁号；只有服务端小区词库精确命中的 `address===community` 存量值可按小区名公开，审核或坐标标记不能自我白名单未知完整地址。
...
- 当前有效房源的公开 DTO 只返回 API 域不透明能力地址，默认有效 6 小时；地址不包含 OSS 对象键、历史文件名、AccessKey 或 OSS 签名参数。`GET /mini/listings/:id/media/:kind` 支持 `GET`、`HEAD` 和单段 `Range`，每次请求都重新核对房源仍为前台有效状态，再由服务端生成默认 900 秒的 OSS 短签名并限长流式转发。媒体源只允许配置中的精确 OSS origin 和受控上传目录，不跟随重定向。
...
地图筛选复用区域、板块、租金、户型和租赁方式等参数，并用 `sourceType` 精确筛选公司/业主/二房东来源，但只返回可靠小区坐标。从地图切回列表时保留当前来源与其他筛选条件；游客公开字段投影与列表一致。
