...
- 只有上一轮完整当前主档中存在、本轮完整源快照中消失的源记录，才按既有生命周期规则进入“已出租”；同时关闭 `published/enabled`，先追加已出租周期和状态流水并严格回读，再更新当前主档。同一物理房源真正更换源 `record_id` 时仍按下文规则关闭旧周期、开启新周期。本轮只调整批量撤下保险丝的统计身份：AI 数据底座按“城市/行政区/板块/小区/楼栋/单元/房号/出租方式”生成的稳定物理房间键统计，并把线上活跃飞书库存作为第二基线；仅服务器旧库存残留上一张源表 ID，而当前专用表与员工源已经一致时，旧 ID 不再单独计作撤下，真实物理房间大批消失仍会在首个写请求前阻断。分页不完整、字段错型、源空表、批量撤下熔断或任一写后回读不一致时，库存与公开十列表都不发布。
