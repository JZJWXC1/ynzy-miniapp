...
看房方式（`viewingMethod`）为选项字段：`钥匙` / `密码` / `联系房东`。公司、业主、二房东房源及三种看房方式都必须提供合法 11 位房东手机号；钥匙方式额外必填 `viewingKeyLocation`，密码方式额外必填 `viewingPassword`。旧客户端不传看房方式也不能绕过手机号校验，飞书“公司统一维护”等占位不再作为合法电话。
