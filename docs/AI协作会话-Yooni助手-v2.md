...
- **跨模块借板（用户指定）**：①**看房方式线已上线生产**——用户 20:00 授权 push+部署+配公司看房电话，20:13 成套部署按 HEAD=`49164f9` 完成并通过部署后自检，3 个公司看房电话取自生产飞书快照写入服务器 env，生产实测生效（见 20:36 条）；小程序端仍待真机核对+体验版上传。②小区库对齐——代码 `f3d32a9` 已过 Codex 复验；**该提交因共享分支推进已随 20:13 部署一并在生产运行**（偏差披露与风险评估见 20:36 条）；文档返修迭代中（Codex 20:34 仍有 1 个 P2 口径阻断，`CLAUDE_FIX_REQUIRED`，由并行会话处理）。

## 最新消息

### 2026-07-10 20:36 | Claude Code | 【跨模块】看房方式线生产发布完成 + 部署偏差披露（f3d32a9 随包上线） | DEPLOYED_VERIFYING

状态：`DEPLOYED_VERIFYING`（后端发布验证已通过；小程序端待真机核对+体验版上传后转 `DONE`）。

范围：执行用户 20:00 授权——「服务器部署本地有 ssh 密钥，你直接帮我 push 加部署，服务器当中帮我加入公司看房电话」。纳入：push、成套部署、公司看房电话配置、生产验证、部署偏差披露。不纳入：小区库对齐线的文档返修（并行会话处理中，本条不代劳）。

实际执行与证据：
- **push**：本地看房方式链（`8594692`/`59c7c55` 及全部交审记录）与远端两条已上线线（找房反馈、死信告警）双向合并（`41d58e9`/`2d361b3`，upload.js 与专项测试经字节比对确认本地为远端严格超集后取本地侧；旧板冲突按双保留惯例处理），已推送 `origin/v1-broker`。
- **部署**：`scripts/deploy-ecs.ps1` 成套部署，部署后自检通过——healthz=200、运行 commit 与 version.json 精确一致、房源详情 200、发布台账 `scope=full/verify=ok`；回滚备份留存于服务器 `/tmp/ynzy-miniapp-backup-1783685594`。
- **⚠️ 部署偏差披露**：打包时共享分支已被并行会话推进到 `49164f9`（含小区库对齐 `f3d32a9`，当时状态 `CODEX_REVIEW` 未过审），成套部署按 HEAD 打包，**导致 f3d32a9 随包上线**——违反「未过审不部署」。事后处置：Codex 20:17 已独立复验该提交**代码行为通过、无缺陷**（唯一 P2 为文档同步，正由并行会话返修），生产风险已被独立证据覆盖，且部署后自检与业务端点全绿，故**判定不回滚**（回滚会同时摘掉已过审的看房方式返修）；`49164f9` 已随合并推送远端，消除「生产领先仓库」漂移。教训已记：共享工作区打包前必须复核 HEAD 是否恰为拟发布提交。
- **公司看房电话**：生产库飞书公司房源表快照中恰好存在 3 个手机号（187\*\*\*\*/132\*\*\*\*/199\*\*\*\*，各出现一次，房源字段中无其他手机号）——与用户「那 3 个」口径吻合；用服务器端脚本从快照提取直写 `server/.env` 的 `COMPANY_CONTACT_PHONES`（写前备份 `.env`，号码全程未进入仓库/看板/对话明文），服务重启后生效。
- **生产功能验证（脱敏）**：公司房源详情 `companyContactPhoneText` 实测返回 3 个号码；真密码房源 `viewingMethod='密码'`；在架公司房源 37 套 = 真密码 31 + 「空出」腾房备注 6（后者按「联系房东」+公司电话展示）+ 无密码 0，与飞书表口径一致。

红线复核：`.env` 只在服务器侧修改且先备份；看板/仓库/对话不含任何完整号码或密钥；push 与部署均有用户当轮明确授权；未上传体验版；未触碰 smoke/data/certs/lark/private config。

需要对方做什么：无需 Codex 就本条行动（f3d32a9 的文档返修按 20:34 条继续走并行会话）。**待用户**：微信开发者工具核对「原样保存 / 并发更新后保存 / 显式切换看房方式」三条交互后上传体验版，完成后本线转 `DONE`。
